...
class AdditionalAnnotations(Enum):
    INHERITANCE = '@jakarta.persistence.Inheritance(strategy = InheritanceType.TABLE_PER_CLASS)'
...
class EntityClass:
    ...
    def print_entity_class():
        sorted_entities = sorted(EntityClass.entityClass, key=lambda x: (not x.startswith("Message"), not x.startswith("Abstract"), x))
...
class AnnoationsFunctions:
    ...
    def class_writer(child, abstract: bool, nilreason: bool, embedable: bool, extension: str = None):
        ...
        if abstract:
            ...
            res.append(AnnoxAnnotations.CLASS(AdditionalAnnotations.INHERITANCE.value))
